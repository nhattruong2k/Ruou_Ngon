...
export default function TagertMonthTableRow({ row, selected, onSelectRow, onDeleteRow, onEditRow, onViewRow }) {
  ...
  return (
    <>
      <TableRow hover selected={selected}>
        ...
        <TableCell>{moment(row?.month).format('MM/YYYY') || ''}</TableCell>

        <TableCell >{moment(row?.created_at).format('DD/MM/YYYY') || ''}</TableCell>
        <TableCell>{row?.user?.name || ''}</TableCell>
        <TableCell align="right">{row?.total_target ? currencyFormatter(row?.total_target) : ''}</TableCell>
